...
- Step 2's second confirmation is excluded-path candidates with evidence; setup writes `## Excluded Paths` in both enforcement modes. The audit produces no findings from an excluded directory.
- Step 3a proposes a commit format from commitlint / git log, writes `## Git` in both modes. `lodestar-fix` honors `commits: never` without asking; a protected branch stops the session and offers to continue without committing.
- Step 1 measures churn with four git/filesystem commands (no source reading). Step 2 shows those numbers and asks one scope question; recommended `changed-since` when files ≥ 80 and 90-day churn < 0.30, else `all`. On `changed-since`, capture `git rev-parse HEAD` and today's date. Not a git repo → `mode: all` without asking. Step 9 names the scope and, when `changed-since`, says the next audit will find little by design and that existing code is the `INDEX.md` backlog.
- A `context.md` with none of the new sections (`## Excluded Paths`, `## Git`, `## Audit Scope`, `Scannable`, `Entry points`) produces unchanged output.
...
- Commit policy comes from `## Git` (defaults if absent): skip the auto-commit question when `per-item` or `never`; `never` leaves edits unstaged; a protected branch stops and offers to continue without committing; a rejecting hook defers the item with the hook output and does not `--no-verify`.
