...
- Setup does **not** write `## Resolved Decisions`. `validate-input` derives `probePlan`, `activeDetectors`, and `blindSpots` in memory from Conventions, Package Layout, Dependency Direction, and the linter cell. An old section is ignored; the next setup re-run deletes it. The package manager is always resolved and recorded before writing `context.md` — no downstream skill asks for it.
