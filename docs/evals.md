...
- `validate-input` requires `## Resolved Decisions` in `context.md`; a file missing the section fails with "Re-run lodestar-setup". It returns `activeDetectors`, `blindSpots`, and `probePlan` derived from conventions and Package Layout. The audit reads `activeDetectors` to decide which subtypes to run — it does not re-evaluate convention gates. Known blind spots in `INDEX.md` start from `validate-input` `blindSpots` (convention-gated, unscannable packages, single-package not-applicable); runtime entries (fallow-optional, coverage) are appended. No mid-audit pkgManager question — setup always resolves and records it; `pkgManagerProvenance: none` is a warning only.
...
- Grounds before writing: every Scope / files path not marked `(new)` must exist; every named command must be a real script, Build & Test cell, or task-file target; assumed imports must match `## Dependency Direction`, or — with no `context.md` — must not create a cycle against today's imports. A miss stops the write.
