#!/usr/bin/env node
/**
 * Deterministic audit-state helper. Installed with ep-audit.
 *
...
export function parsePackageLayout(agentsText) {
  ...
  for (const row of rows) {
    ...
    if (reason) {
      throw new Error(
        `Package \`${row.name}\` has no real Responsibility. ${reason} Re-run ep-setup and write a concrete one-sentence description. The audit relies on this column to judge boundary findings and won't run with a placeholder.`,
      );
    }
  }
  return rows;
